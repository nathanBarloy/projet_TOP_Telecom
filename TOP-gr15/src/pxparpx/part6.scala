...
object part6 {
	...
	/********* programme générant l'histogramme intégral d'une image ********/
	...
	def histoIntegral(img:Array[Array[Int]]): Array[Array[Array[Int]]] = {
	  ...
	  var imgGris = pxparpx.part1.transfoTableGris(img)
	...
	/********** programmes de calculs sur les histogrammes ********/
	...
	/*********** programme extrayant l'histogramme de la sous-image de taille (m,n) commençant à (i,j) *******/
	...
	def extractHistoIntegral(hist_int: Array[Array[Array[Int]]], i:Int, j:Int, m:Int, n:Int):Array[Int]={
		...
		if(i == 0 && j == 0){
		  ...
		  resultat =  hist_int(m-1)(n-1)   //L'histogramme correspond déjà si le début de la sous_image est (0,0)
		  
		}else if(i ==0){  //On distingue les cas pour les calculs
		  
		  var ajout = hist_int(m-1)(n+j-1); //Histogrammes en somme positive dans le calcul : le "plus grand" et/ou le "plus petit"
		  ...
		  var ajout = hist_int(m+i-1)(n-1); //Histogrammes en somme positive dans le calcul : le "plus grand" et /oule "plus petit"
		  var substrat = hist_int(i-1)(n-1); //Histogrammes en somme négative (soustraction) dans le calcul : les effets de bord
		  resultat = soustrHisto(ajout,substrat);  
